...
export default function PdfDropzone({ files, setFiles, outlines,setOutlines, onPdfClick }) {
    ...
    async (acceptedFiles) => {
      for (let file of acceptedFiles) {
        if (files.some((f) => f.file.name === file.name)) {
          ...
          if (action === "overwrite") {
            try {
              await fetch(`http://localhost:8000/delete/${file.name}`, { method: "DELETE" });
        ...
        try {
          ...
          if (data.error) {
            ...
            setFiles((prev) => [
              ...prev,
              { file, url: `http://localhost:8000/uploads/${file.name}` },
            ]);
            setNewlyAdded(prev => new Set(prev).add(file.name));
            // After setFiles and setNewlyAdded
          try {
            const procRes = await fetch(
              `http://localhost:8000/api/v1/extract-outline/?file_name=${file.name}`
            );
  ...
  const handleDelete = async (name) => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/delete/${name}`, {
        method: "DELETE",
      });
      const data = await res.json();
