  ...
  (async () => {
    try {
      ...
      for (let fname of data.files) {
        loaded.push({
          file: { name: fname },
          url: `http://localhost:8000/uploads/${fname}`,
        });

        try {
          // Request outline from backend for this file
          const procRes = await fetch(`http://localhost:8000/api/v1/extract-outline/?file_name=${fname}`);
...
useEffect(() => {
  ...
  const trainModel = async () => {
    try {
      ...
      await fetch("http://localhost:8000/relevant/train", { method: "POST", signal });

      interval = setInterval(async () => {
        try {
          const res = await fetch("http://localhost:8000/relevant/train/status", { signal });
          const data = await res.json();
