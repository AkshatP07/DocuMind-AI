...
useEffect(() => {
  const fetchData = async () => {
    try {
      ...
      const relevantRes = await axios.get("http://127.0.0.1:8000/relevant/search", {
        params: {
          query: selectedText,
          k: 15,
          context: 1,
...
`;
      ...
      // ---- Retry wrapper for Gemini call ----
      let insightRes;
      const maxRetries = 2; // first try + 2 retries
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
          insightRes = await axios.post(
          ...
          console.warn(`Gemini call failed (attempt ${attempt + 1}), retrying...`);
                    ...
                    onClick={() => {
                      if (!selectedPdf || selectedPdf.file.name !== item.doc_id) {
                        setSelectedPdf({
                          ...
                          url: `http://localhost:8000/uploads/${item.doc_id}`,
                        });
                        setTimeout(() => {
